...
public class Ascension extends PApplet implements Serializable {
  ...
  public Ascension(int squareSize, int depth, int numColors, int initSize,
      int tileSize) {
    ...
    for (int i = 0; i < numColors; i++) {
      tiles.add(new Tile(colorArray, tileSize, PROTOTILE_SQUARE_SIZE, "tile "
          + i));
    }
  }
  ...
  public void addTile() {
    Tile tile = new Tile(colorArray, tileSize, PROTOTILE_SQUARE_SIZE, "tile "
        + tiles.size());
    tiles.add(tile);
  }
  ...
  /*
   * Given a number of iterations, estimate the maximum square size that can be
   * used with that number of iterations.
   */
  public int estSquareSizeMax(int depth) {
    return (int) Math.floor(Math.sqrt(EST_MAX_PIX)
        / (calculateImgWidthSquares(initSize, tileSize, depth)));
  }

  /*
   * Given a square size, estimate the maximum number of iterations that can be
   * completed with that square size.
   */
  public int estDepthMax(int squareSize) {
    return (int) Math.floor(Math.log(Math.sqrt(EST_MAX_PIX)
        / (squareSize * initSize))
        / Math.log(tileSize));
  }

  /*
   * After any changes have been made to Ascension parameters, completeResize()
   * must be called to recalculate image sizes.
   */
  public void completeResize() {
    ...
    System.out.println("width in squares " + imgWidthSquares + " width in pix "
        + imgWidthPix);
    size(imgWidthPix, imgWidthPix);
    img.resize(imgWidthPix, imgWidthPix);
    System.out.println("completed resize to " + img.pixels.length);
  }

  public static int calculateImgWidthSquares(int initSize, int tileSize,
      int depth) {
  ...
  public void setAnimationParameters(int startX, int startY, int transX,
      int transY) {
    slideX = startX;
    slideY = startY;
    this.startX = startX;
  ...
  /*
   * Start iterating with the initMatrix. For each square in the initial matrix,
   * execute drawPixRecurse with depth.
   */
  ...
  /*
   * If depth equals 0, draw square of color v in position i, j. Otherwise, for
   * each square in the matrix corresponding to color v, execute drawPixRecurse
   * with depth-1.
   */
    ...
    if (depth == 0) {
      for (int k = 0; k < squareSize; k++) {
        for (int l = 0; l < squareSize; l++) {
          try {
            img.pixels[i * squareSize + k + (j * squareSize + l) * imgWidthPix] = colorArray
                .get(v);
          } catch (ArrayIndexOutOfBoundsException e) {
            System.out.println("ERROR array out of bounds on " + i + " " + j
                + " " + k + " " + l + " "
                + (i * squareSize + k + (j * squareSize + l) * imgWidthPix)
                + " " + img.pixels.length);
            System.exit(1);
          }
        }
  ...
  /*
   * Returns when the current drawing cycle is complete. To be used before
   * changing Ascension parameters. For example:
   *   ascension.noLoop();
  ...
  public static void main(String[] args) {
    ...
    if (args.length > 0) {
      if (args[0].equals("-split")) {
        displayFrame = ControlPanel.initJFrame("Ascension", 0, 0, 1000, 500);
        TileAnimationPanel dsp = TileAnimationPanel.init(asc);
        ControlPanel cp = ControlPanel.init(asc, dsp);
        JSplitPane jSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, cp, dsp);
        ...
        try {
          displayFrame = ControlPanel.setUpFrames(asc, TilePanel.class, cpW, 0,
              dpW - 50, dpH, 0, 0, cpW, cpH, 0, cpH + 22, cpW, spH);
        } catch (Exception e) {
          e.printStackTrace();
          System.exit(1);
        }
      ...
      try {
        displayFrame = ControlPanel.setUpFrames(asc, TileAnimationPanel.class,
            cpW + spW, 0, dpW, dpH, 0, 0, cpW, dpH, cpW, 0, spW, dpH);
      } catch (Exception e) {
        e.printStackTrace();
        System.exit(1);
      }
